...
class AlipaySubmit {
	...
	function buildRequestMysignqr($para_sort) {
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
    	//Rearrange parameters in the data set alphabetically and connect rearranged parameters with & like "parametername=value"
		$prestr = createLinkstring($para_sort);
		...
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				$mysign = md5Sign($prestr, $this->alipay_config['key-qr']);
				break;
			default :
				$mysign = "";
		}
    ...
    function buildRequestParaqr($para_temp) {
		//除去待签名参数数组中的空值和签名参数
		//Remove the blank ,sign and sign_type
		$para_filter = paraFilter($para_temp);
		...
		//生成签名结果
		//Generate the sign
		$mysign = $this->buildRequestMysignqr($para_sort);
    ...
    function buildRequestFormqrpay($para_temp, $method, $button_name) {
        //待请求参数数组pre-request params
        $para = $this->buildRequestParaqr($para_temp);
    ...
    function buildRequestFormqr($para_temp, $method, $button_name) {
        //待请求参数数组pre-request params
        $para = $this->buildRequestParaqr($para_temp);
        $params1="";
        ...
        $sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."_input_charset=".trim(strtolower($this->alipay_config['input_charset']))."' method='".$method."'>";
        while (list ($key, $val) = each ($para)) {
            if($key=='extend_params'){
                $params1 .= $key.'='.urlencode($val).'&';
            } else {
                $params1 .= $key.'='.$val.'&';
            }
            // $params1 .= $key.'='.$val.'&';
