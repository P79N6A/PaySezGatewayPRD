							...
							<tbody>
								...
								<tr>
									...
									<td>
			                            ...
			                            } else if($trans['transaction_type'] == 2) {
			                                $transaction_type = 'POS - REFUND';
			                            } else if($trans['transaction_type'] == 3) {
			                                $transaction_type = 'POS - QUERY';
			                            } else if($trans['transaction_type'] == 4) {
			                                $transaction_type = 'POS - CANCEL';
			                            } else if($trans['transaction_type'] == 's1') {
			                                $transaction_type = 'QR - PURCHASE';
			                            } else if($trans['transaction_type'] == 's2') {
			                                $transaction_type = 'QR - REFUND';
			                            } else if($trans['transaction_type'] == 's3') {
			                                $transaction_type = 'QR - QUERY';
			                            } else if($trans['transaction_type'] == 's4') {
			                                $transaction_type = 'QR - CANCEL';
			                            } else if($trans['transaction_type'] == 'cb1'){
			                                $transaction_type = 'CBP - SALE';
			                            }else if($trans['transaction_type'] == 'cb2') {
			                                $transaction_type =  'CBP - REFUND';
			                            } else if($trans['transaction_type'] == 'cb3'){
								...
								<tr>
									...
									<td>
									<?php 
									if($trans['transaction_type'] == 1 || $trans['transaction_type'] == 4 || $trans['transaction_type'] == 's1' || $trans['transaction_type'] == 's4'|| $trans['transaction_type'] == 's3' || $trans['transaction_type'] == 's3'||$trans['transaction_type'] == 'cb1'||$trans['transaction_type'] == 'cb3') {
										echo $trans["currency"].' '.number_format($trans["total_fee"],2);
									} else {
										echo $trans["currency"].' '.number_format($trans["refund_amount"],2);
									} 
									?>	
									</td>
								</tr>
								...
								<tr>
									<th>Status</th>
									<td><?php
										if($trans['transaction_type'] == 1 || $trans['transaction_type'] == 's1'||$trans['transaction_type'] == 'cb1'||$trans['transaction_type'] == 'cb3') {
	                                        ...
	                                        else echo "ACK_NOT_RECEIVED";
	                                    } else if($trans['transaction_type'] == 2 || $trans['transaction_type'] == 4 || $trans['transaction_type'] == 's2' || $trans['transaction_type'] == 's4'|| $trans['transaction_type'] == 'cb2') {
	                                    	if($trans['result_code'] =='SUCCESS'||$trans['refund_status'] == 'REFUND_SUCCESS') {
	                                    		echo 'SUCCESS';
	                                    	} else {
	                                    		echo "Failed";
	                                    	}
								...
								<tr>
									<th>Response</th>
									<td><?php if($trans['transaction_type'] == 'cb2'){
										echo $trans['refund_status']; } else {
											echo $trans["trade_status"]; 
										} ?></td>
								</tr>
							</tbody>
						</table>
						...
						<form id="vtrefundform">
							...
							<div class='row'>
								...
								<div class='col-xs-12 form-group threebtns'>
									...
									<?php if($trans["merchant_id"] == 'E01010000000042') { ?>
									<div class="col-xs-12 form-group no-padding">
										<label class="control-label">Refund Reason<span class="text-danger">*</span></label>
										<?php if($cancel_flag >= 1 || $refund_flag >= 1) { ?>
										<p><?php echo $data_refund[0]['refund_reason']; ?></p>
										<?php } else { ?>
										<input type="text" name="ref_reason" id="ref_reason" class="form-control" value="" placeholder="Refund Reason">
										<?php } ?>
									</div>
									<?php } ?>
                ...
                confirm: function () {
                    ...
                    $.ajax({
                        ...
                        data: {from:'pg', action: 's2', device: 'QR', currency: '<?php echo $trans['currency']; ?>', return_amount: '<?php echo $trans['total_fee']; ?>', partner_refund_id:'<?php echo
                            $trans['terminal_id'].date('YmdHis'); ?>', partner_trans_id: '<?php echo $trans['out_trade_no']; ?>', merchantid: '<?php echo $trans['merchant_id']; ?>', terminal_id: '<?php echo $trans['terminal_id']; ?>', ref_reason: refreason},
                        success: function(response) { 
				          $("#pageloaddiv").hide();
				          // alert("Success");			
				        }
                        ...
                        if(name.match(rgx)==null){
                            ...
                            if(name <= tot){
                                ...
                                $.ajax({
                                    ...
                                    url: "alipayapi_qr.php",
                                    dataType: "json",
                                    data: {from:'pg', action: 's2', device: 'QR', currency: '<?php echo $trans['currency']; ?>', return_amount: name, partner_refund_id:'<?php echo $trans['terminal_id'].date('YmdHis'); ?>', partner_trans_id: '<?php echo $trans['out_trade_no']; ?>', merchantid: '<?php echo $trans['merchant_id']; ?>', terminal_id: '<?php echo $trans['terminal_id']; ?>', ref_reason: refreason},
			                        success: function(response) { 
							          $("#pageloaddiv").hide();
							          // alert("Success");			
							        }
