...
// $enco = http_build_query($response_data);


if($tran_req_type == 1){
    $Async_succs = date("Y-m-d H:i:sa") . "\n---------------------\nAsync response log send to pos:\n" . json_encode($response_data) . " \n\n";
            $myfile = file_put_contents('/var/www/html/testspaysez/api/alipaytranLOG.log', $Async_succs . PHP_EOL, FILE_APPEND | LOCK_EX);

    $curl = curl_init();

    curl_setopt_array($curl, array(
      CURLOPT_PORT => "8080",
      CURLOPT_URL => "https://123.231.14.207:8080/AliPayCallBack/CallBack",//"http://220.247.222.76:8080/AliPayCallBack/CallBack",
...
// $myfile = file_put_contents('/var/www/html/testspaysez/api/alipaytranLOG.log', $resp . PHP_EOL, FILE_APPEND | LOCK_EX);
