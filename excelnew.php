...
// Create new PHPExcel object
          ...
          $sql_query ="SELECT * FROM merchants INNER JOIN merchant_processors_mid ON merchant_processors_mid.mer_map_id = merchants.mer_map_id WHERE merchants.mer_map_id ='GPT0000001'";
          ...
          // Rename sheet
           ...
           //echo "<pre>";
            $db->where('mer_map_id','GPT0000001');
            ...
            foreach ($Terminal_ids as $details){
                 ...
                 $objPHPExcel->setActiveSheetIndex(0)
                             ...
                             ->setCellValue('I'.$i, 'Grapay(1%)')
                             ->setCellValue('J'.$i, 'Revopay');
                  ...
                  $terminal_id = $details['mso_terminal_id'];
                      // echo $terminal_id;
                      // echo "<br>";
                  $sql ="SELECT * FROM gp_transaction WHERE gp_merchant_id ='GPT0000001' AND  gp_transaction_type='1' AND gp_status='success' AND gp_terminal_id='$terminal_id'";
                  ...
                  if (!empty($details1)) {
                                    ...
                                    foreach($details1 AS $subrow) {
                                          ...
                                          $objPHPExcel->getActiveSheet()
                                           ...
                                           $objPHPExcel->setActiveSheetIndex(0)
                                                        ...
                                                        ->setCellValue('H'.$i,$prev_date)
                                                        ->setCellValue('I'.$i, $subrow['grabpay_amount']=getPercentOfNumber($subrow['gp_amount'],1))
                                                        ->setCellValue('J'.$i, $subrow['revopay_amount']=$subrow['mdr_amount']-$subrow['grabpay_amount']);
                                                          $sum+=$subrow['gp_amount'];
                                                          $summdr+=$subrow['mdr_amount'];
                                                          $sumnet+=$subrow['net_amount'];
                                                          $sumdate+=$prev_date;
                                                          $sumgrabpay+=$subrow['grabpay_amount'];
                                                          $sumrevopay+=$subrow['revopay_amount'];
                        ...
                        $objPHPExcel->getActiveSheet()
                         ...
                         $objPHPExcel->setActiveSheetIndex(0)
                                     ...
                                     ->setCellValue('I'.$i, $sumgrabpay)
                                     ->setCellValue('J'.$i, $sumrevopay)
        ...
        // Redirect output to a client’s web browser (Excel5)
          ...
          header('Content-Disposition: attachment;filename="Merchant_id(GPT0000001).xls"');
