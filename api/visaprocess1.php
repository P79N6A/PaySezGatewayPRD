...
if($_POST["transType"]=="02" && $_POST["revResCode"]=='32') {
    ...
    foreach ($_POST as $key => $value) {
        if ($key != "appredirect" && $key != "action_id" && $key != "redirectionurl" && $key != "env" && $key != "revDateAndTime" && $key != "paymentIndicator" && $key != "revResCode" && $key != "partialRevAmt") {
...
//print_r($my_array);
exit;*/
$logtosend='postvalue='.$postdatalog.'&env='.$_POST["env"];
$ch = curl_init();
...
if(curl_errno($ch))
{
    ...
    exit;
    //$my_array->refNbr;*/
    $logtosend='postvalue='.$output.'&env='.$_POST["env"];
    $ch = curl_init();
    // curl_setopt($ch, CURLOPT_URL, "http://169.38.91.251/api/loginsert.php");
    curl_setopt($ch, CURLOPT_URL, "http://10.162.104.221/api/loginsert.php");
    curl_setopt($ch, CURLOPT_POST, 1);
    ...
    */
    ...
    if($respCode == "00"){
        ...
        if($_POST['env']=='livem' || $_POST['env']=='testm'){
            ...
            // http://169.38.91.251/api/savetraninfo.php
            header('Location: http://169.38.91.250/api/savetraninfo.php?tid=' . $tid . '&transactionid=' . $transactionid . '&terminalId=' . $terminalId . '&authNbr=' .
                $authNbr . '&refNbr='
