...
$logtosend='postvalue='.$log.'&env=live'; //.$_POST['env'];
$ch = curl_init();
...
$servername = "10.162.104.221"; // "localhost";
$username = $userd;
$password = $passd;
$dbname = "rebanx";
...
//$trandt=$trans_date_time;
$trans_date_time=date('mdHis');
...
?>
<form id="myFormvisa" action="http://169.38.91.250/api/visaprocess1.php" method="post">
 ...
 <input type="hidden" name="merchLocation" value="<?php echo $card_acc_name_loc; ?>">
    <input type="hidden" name="MCC" value="<?php echo $mcc; ?>">
            ...
            $logtosend='postvalue='.$log.'&env=live'; //.$env;
            $ch = curl_init();
            // curl_setopt($ch, CURLOPT_URL, "http://169.38.91.251/api/loginsert.php");
            curl_setopt($ch, CURLOPT_URL, "http://10.162.104.221/api/loginsert.php");
            curl_setopt($ch, CURLOPT_POST, 1);
