...
// {"code":4040,"reason":"transaction not found"}
    ...
    // $edate = date('Y-m-d 23:59:59',strtotime("-1 days"));
        ...
        $que1 ="SELECT gp_terminal_id,gp_merchant_id, COUNT(gp_transaction.gp_transaction_id) AS countt, SUM(gp_transaction.gp_amount) AS total FROM gp_transaction WHERE gp_merchant_id = '$mer_id' AND gp_terminal_id = '$ter_id' AND gp_transaction_type IN ('1') AND gp_status='success' AND gp_cst_trans_datetime>='$sdate' AND gp_cst_trans_datetime<='$edate'";

        $data1 = $db->rawQuery($que1);

        $que2 ="SELECT gp_terminal_id,gp_merchant_id, COUNT(gp_transaction.gp_transaction_id) AS countt, SUM(gp_transaction.gp_amount) AS total FROM gp_transaction WHERE gp_merchant_id = '$mer_id' AND gp_terminal_id = '$ter_id' AND gp_transaction_type IN ('2') AND gp_status='success' AND gp_cst_trans_datetime>='$sdate' AND gp_cst_trans_datetime<='$edate'";
...
elseif ($_POST['payment_type'] == 'AP'):
    ...
    $vendor_payment_type = $_POST['payment_type'];
    //partner id & partner secret for alipay
    // $db->where("mer_map_id" , $_POST['merchant_id']);
    // $merchant_result = $db->getOne("merchants");
    // $partner_id_ap = $merchant_result['alipay_partner_id'];
    // $partner_key_ap = $merchant_result['alipay_partner_key'];

    $db->where("vendor_name" , $vendor_payment_type);
    $merchant_result = $db->getOne("vendor_config");
    $partner_id_ap = $merchant_result['vendor_username'];
    $partner_key_ap = $merchant_result['vendor_secretkey'];
    ...
    switch ($tran_req_type) {
        ...
        case "1": //QR request
            ...
            exit;
    case "2": //Refund
            $ref_terminal_id = $_POST['terminal_id'];
            $ref_merchant_id = $_POST['merchant_id'];
            $ref_total_fee = $_POST['amount'];
            $ref_currency = $_POST['currency'];
            $ref_out_trade_no_orig = $_POST['orig_partner_txid'].$partner_substr_ap;
            $ref_partner_refund_id = $partnerTxID_ap;
            $ref_partner_id_res_ap = $_POST['orig_partner_txid'];
            $ref_partner_id_ap = $partner_id_ap;
            $ref_partner_key = $partner_key_ap;

            /* Refund request data log creation ,received from POS */

            $ref_req_data_pos = "Application Log POS:".date("Y-m-d H:i:s") . " Refund request data from pos:" . json_encode($_POST) . " \n\n";
            poslogsAlipay($ref_req_data_pos);

            refund($ref_terminal_id,$ref_merchant_id,$ref_total_fee,$ref_currency,$ref_out_trade_no_orig,$ref_partner_refund_id,$ref_partner_id_res_ap,$ref_partner_id_ap,$ref_partner_key,$tran_req_type);
            
            break;
            exit;
    case "4"://Query
            $qry_out_trade_no_orig = $_POST['partnerTxnid_orig'].$partner_substr_ap;
            $qry_terminal_id = $_POST['terminal_id'];
            $qry_currency = $_POST['currency'];
            $qry_type = $_POST['txType'];
            $qry_merchant_id = $_POST['merchant_id'];
            $qry_partner_id_res_ap = $_POST['partnerTxnid_orig'];
            $qry_partner_id_ap = $partner_id_ap;
            $qry_partner_key = $partner_key_ap;

            /* Query request data log creation ,received from POS */

            $qry_req_data_pos = "Application Log POS:".date("Y-m-d H:i:s") . " Query request data from pos:" . json_encode($_POST) . " \n\n";
            poslogsAlipay($qry_req_data_pos);
            
            query($qry_terminal_id,$qry_merchant_id,$qry_currency,$qry_out_trade_no_orig,$qry_partner_id_ap,$qry_partner_id_res_ap,$qry_partner_key,$tran_req_type);

            break;
            exit;
    case "3"://Cancel   
            $cncl_out_trade_no_orig = $_POST['orig_partner_txid'].$partner_substr_ap;
            $cncl_terminal_id = $_POST['terminal_id'];
            $cncl_currency = $_POST['currency'];
            $cncl_merchant_id = $_POST['merchant_id'];
            $cncl_partner_id_ap = $partner_id_ap;
            $cncl_partner_key = $partner_key_ap;
            $cncl_out_trade_no_split = $_POST['orig_partner_txid'];

            $cncl_req_data_pos = "Application Log POS:".date("Y-m-d H:i:s") . " Cancel request data from pos:" . json_encode($_POST) . " \n\n";
            poslogsAlipay($cncl_req_data_pos);

            cancel($cncl_terminal_id,$cncl_merchant_id,$cncl_currency,$cncl_out_trade_no_orig,$cncl_partner_id_ap,$cncl_partner_key,$cncl_out_trade_no_split,$tran_req_type);
            break;
            exit;
    ...
    case "5": //settlement 
            ...
            $payment_type = $_POST['pay_type'];
            $settle_mer_id = $_POST['merchant_id'];
            $settle_terminal_id = $_POST['terminal_id'];

             $settle_received_data = "Application Log POS:".date("Y-m-d H:i:s") . " Settlement Received Data from Mpos :" . json_encode($_POST) . " \n\n";
            poslogsAlipay($settle_received_data);

            settlementAlipay($settle_terminal_id,$settle_mer_id,$payment_type);
