...
if($_GET['start_d']!='') {
    ...
    $query="SELECT * FROM merchants JOIN gp_transaction ON gp_transaction.gp_merchant_id = merchants.mer_map_id AND merchants.userid= '$iid' AND gp_transaction.gp_transaction_type NOT IN ('4','s3') AND gp_transaction.gp_trans_datetime >= '$val1_excel' AND gp_transaction.gp_trans_datetime <= '$val2_excel' ORDER BY gp_transaction.gp_trans_datetime DESC";
    ...
    <TABLE width="100%" cellpadding="1" border="1" cellspacing="0">
        <tr>
            ...
            <td width="15%"><div id="wt_txt" align="center"><b>Transaction ID</b></div></td>
            <td width="15%"><div id="wt_txt" align="center"><b>Refund Orginal ID</b></div></td>
        ...
        <?php $i=0; foreach($transactions_Excel as $tr) { $i++; ?>
            ...
            } else if($tr['gp_transaction_type'] == 's1') {
            ...
            if($tr['gp_transaction_type'] == 2 || $tr['gp_transaction_type'] == 's2'||$tr['gp_transaction_type'] == 'cb2') {
                        $transaction_amount = number_point($tr["gp_amount"]/100);
                } else {
                    $transaction_amount = number_point($tr["gp_amount"]/100);   
                }
            ...
            <tr>
                ...
                <td align="center" id="lt_edit"><?php echo $tr["gp_partnerTxID"]; ?></td>
                <td align="center" id="lt_edit"><?php echo $tr["gp_orig_partnerTxID"]; ?></td>
                <td align="center" id="lt_edit"><?php if($tr['gp_terminal_id']!="") { echo $tr['gp_terminal_id']; } else { echo "--"; }?></td>
                <td align="center" id="lt_edit"><?php echo $sta; ?></td>
                <td align="center" id="lt_edit"><?php echo $tr["gp_cst_trans_datetime"]; ?></td>
                <td align="center" id="lt_edit"><?php echo $tr["gp_currency"].' '.$transaction_amount; ?></td>
            </tr>
    ...
    if($merchantid==0)
    {
        ...
        if($val1!='' && $val2!='' && $merchantid!=0){
            ...
            if(isset($_POST['M_Date']) || $_POST['M_Date']!='') {
                ...
                $query="SELECT gp_transaction.gp_transaction_id, gp_transaction.gp_merchant_id, gp_transaction.gp_partnerTxID,gp_transaction.gp_status, gp_transaction.gp_currency, gp_transaction.gp_amount, gp_transaction.gp_trans_datetime FROM merchants JOIN gp_transaction ON gp_transaction.gp_merchant_id = merchants.mer_map_id AND merchants.userid= '$iid' AND gp_transaction.gp_trans_datetime >= '$first_date' AND gp_transaction.gp_trans_datetime <= '$last_date'";
                ...
                if(!isset($_GET['date'])) {
                    if(!empty($transactions)){
                        ...
                        foreach($transactions as $tr) {
                            ...
                            $t_id = $tr["gp_transaction_id"]; // $_GET['t_id'];
                            ...
                            $result .= '<tr class="gradeX">
                                            <td>'.$tr["gp_transaction_id"].' / '.$transaction_type.'</td>
                                           <td>'.custom_echo($trans_out_trade_no,12).'</td>
                                            ...
                                            <td align="center"><a href="transactiondetails_popup.php?t_id='.$tr["gp_transaction_id"].'" class="load-modal" data-toggle="modal" data-target="#myModal2'.$tr["id_transaction_id"].'" title="Click To View Details"><i class="glyphicon glyphicon-plus-sign" style="font-size: 20px;"></i></a></td>
                                        </tr>';


                            $result .= '<div id="myModal2'.$tr["gp_transaction_id"].'" class="modal fade">
            ...
            if(isset($_POST['S_Date']) || $_POST['S_Date']!='' || isset($_GET['date']) || $_GET['date']!='') {
                ...
                // echo "<br>Hi12_2";
                $query="SELECT * FROM merchants JOIN gp_transaction ON gp_transaction.gp_merchant_id = merchants.mer_map_id AND merchants.userid= '$iid' AND gp_transaction.gp_transaction_type NOT IN ('4','s3') AND gp_transaction.gp_trans_datetime >= '$val1' AND gp_transaction.gp_trans_datetime <= '$val2'";
                ...
                function custom_echo($x, $length) {
                        if(strlen($x)<=$length) {
                               ...
                               // echo "<br>Hi12_2";
                 $query="SELECT * FROM merchants JOIN gp_transaction ON gp_transaction.gp_merchant_id = merchants.mer_map_id AND merchants.userid= '$iid' AND gp_transaction.gp_transaction_type NOT IN ('4','s3') AND gp_transaction.gp_trans_datetime >= '$val1' AND gp_transaction.gp_trans_datetime <= '$val2' ORDER BY gp_transaction.gp_trans_datetime DESC";
                        ...
                        foreach($transactions as $tr) {
                           ...
                           if($tr['gp_transaction_type'] == 1 || $tr['gp_transaction_type'] == 's1') { 
                            ...
                            $result .= '<tr class="gradeX">
                                            ...
                                            <td>'.custom_echo($tr["gp_terminal_id"],20).'</td>                          
                                            <td>'.$sta.'</td>
                                            <td>'.$tr["gp_cst_trans_datetime"].'</td>
                                            <td>'.$tr["gp_currency"].' '.$transaction_amount.'</td>                         
                    ...
                    <TABLE width="100%" cellpadding="1" border="1" cellspacing="0">
                        ...
                        <?php $i=0; foreach($transactions_Excel as $tr) { $i++; ?>
                            ...
                            <tr>
                                ...
                                <td align="center" id="lt_edit"><?php echo $tr["gp_partnerTxID"]; ?></td>
                                <td align="center" id="lt_edit"><?php echo $tr["gp_orig_partnerTxID"]; ?></td>
                                <td align="center" id="lt_edit"><?php if($tr['terminal_id']!="") { echo $tr['gp_terminal_id']; } else { echo "--"; }?></td>
                                <td align="center" id="lt_edit"><?php echo $sta; ?></td>
                                <td align="center" id="lt_edit"><?php echo $tr["gp_cst_trans_datetime"]; ?></td>
                                <td align="center" id="lt_edit"><?php echo $tr["gp_currency"].' '.$transaction_amount; ?></td>
                            </tr>
