      ...
      if($_GET['merchant_id']) {
          ...
          //             ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            ...
            // Rename sheet
             ...
             //echo "<pre>";
              ...
              $partner_id = $db->get('alipay_config');
              ...
              foreach ($Terminal_ids as $details){
                   ...
                   $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    $terminal_id = $details['mso_terminal_id'];
                        // echo $terminal_id;
                        // echo "<br>";
                    $sql ="SELECT * FROM transaction_alipay WHERE trans_date >= '$start_date' AND trans_date <= '$end_date' AND merchant_id ='$merchant_id' AND  transaction_type='1' AND result_code='SUCCESS' AND trade_status='TRADE_SUCCESS' AND terminal_id='$terminal_id' order by cst_trans_datetime";
                    ...
                    if (!empty($details1)) {
                                      ...
                                      foreach($details1 AS $subrow) {
                                            ...
                                            //echo "<br>";
                                        $subrow['out_trade_no'] = custom_echo($subrow['out_trade_no'],12);
                                            ...
                                            $objPHPExcel->getActiveSheet()
                                             ...
                                             $objPHPExcel->setActiveSheetIndex(0)
                                                          ->setCellValue('A'.$i, $r)
                                                          ->setCellValue('B'.$i, "alipay")
                                                          ->setCellValueExplicit('C'.$i, $subrow['out_trade_no'], PHPExcel_Cell_DataType::TYPE_STRING)
                                                          ->setCellValue('D'.$i, $subrow['terminal_id'])
                                                          ->setCellValue('E'.$i, $subrow['cst_trans_datetime'])
                                                          ->setCellValue('F'.$i, "sale")
                                                          ->setCellValue('G'.$i, $subrow['total_fee'] = $subrow['total_fee'])
                                                          ->setCellValue('H'.$i, $subrow['mdr_amount']=getPercentOfNumber($subrow['total_fee'],$grabpay_MDR['1']))
                                                          ->setCellValue('I'.$i, $subrow['net_amount']=$subrow['total_fee']-$subrow['mdr_amount'])
                                                          ->setCellValue('J'.$i,"---");
                                                            $sum+=$subrow['total_fee'];
         ...
         // $filepath = "https://paymentgateway.test.credopay.in/testspaysez/grabpay/admin/";
            ...
            // $objWriter->save('php://output');
              ...
              header('Content-Type: application/vnd.ms-excel');
              header('Content-Disposition: attachment;filename="Merchant_Transaction_Details_For_Alipay('.$merchant_id.').xls"');
              header('Cache-Control: max-age=0');
              $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
              $objWriter->save('php://output');
            //return $filepath;
