...
if($_GET['merchant_id']) {
      ...
      // $sql ="SELECT count(gp_transaction_id) as trans_count,sum(gp_amount) as total  FROM gp_transaction WHERE gp_merchant_id ='$merchant_id' AND  gp_transaction_type='1'  AND gp_status='success' AND gp_cst_trans_datetime = '$date'";

      // $details = $db->rawQuery($sql);

      // $sql_query ="SELECT count(gp_transaction_id) as trans_count,sum(gp_amount) as total  FROM gp_transaction WHERE gp_merchant_id ='$merchant_id' AND gp_transaction_type='2' AND gp_status='success' AND gp_cst_trans_datetime = '$date'";
      ...
      $details_alipay['0']['trans_count'] = 0;

      $details_alipay['total'] = number_point($details_alipay['0']['total']/100);
      ...
      $details_refund['total']=number_point($details_refund['0']['total']/100);
      ...
      ///grabpay_refund-End--------------------------------//
            ...
            $filename = 'merchant_summary_for_Grabpay_'.date('Y-m-d').'.pdf';  
            $dompdf = new Dompdf();
