...
// $milliseconds = sprintf("%03d",$micro_date_exp[1]);
...
// $Receive_data = "Load Test Receive Log:".date("Y-m-d H:i:s")." ".$milliseconds." Received Data from Terminal POS:" .json_encode($_POST). " \n\n";
// poslogs($Receive_data);
...
// $random_num_arr = randomGen(0,50,1); //generates 20 unique random numbers
// $random_num     = sprintf("%03d", $random_num_arr[0]);

$_POST['device'] = "pos";
$_POST['WIDproduct_code'] = "OVERSEAS_MBARCODE_PAY";
$_POST['WIDsubject'] = "Alipay";
$_POST['terminal_id'] = "E0000001";
$_POST['amount'] = 1;
$_POST['currency'] = "USD";
// $_POST['out_trade_no'] = "E0000001".date('Ymdhis').$random_num; // "E000000120190124124611019";
$_POST['out_trade_no'] = "E0000001".$date->format('YmdHisu'); // "E000000120190124124611019";
$_POST['callback_notify_url'] = "https://123.231.14.207:8080/AliPayCallBack/CallBack";
$_POST['terminal_timestamp'] = date('Ymdhis');
$_POST['tran_req_type'] = 1;

$Receive_data_1 = "Load Test Receive POST Data Log:".$date->format('Y-m-d H:i:s.u')." Received Data from Terminal POS: Out_trade_no : " .$_POST['out_trade_no']. " \n\n";
poslogs($Receive_data_1);
...
// $Receive_data = "Load Test Receive Log:".date("Y-m-d H:i:s") . " Received Data from Terminal POS:" .json_encode($_POST). " \n\n";
// poslogs($Receive_data);
...
// $Receive_data = "Load Test 1 Receive POST Data Log:".date("Y-m-d H:i:s") . " Received Data from Terminal POS:" .json_encode($_POST). " \n\n";
// poslogs($Receive_data);
...
/* terminal & merchant validation starts */
    ...
    //pos test 210918
        ...
        $log = "Application Log POS:".$date->format('Y-m-d H:i:s.u')." Build Data send to Alipay:" . $html_text. " \n\n";
         poslogs($log);
        ...
        if(curl_errno($ch)){
            $log = date("Y-m-d H:i:s") . " Pre-create payment Response POS:" . curl_errno($ch) . " \n\n";
             poslogs($log);

        } else {
            $log = date("Y-m-d H:i:s") . " Pre-create payment Response POS:" . $server_output . " \n\n";
             poslogs($log);
        }
        ...
        $final_pay_res = date("Y-m-d H:i:s") . " Fianl payment Response update in table POS:" . json_encode($data) . " \n\n";
 ...
 //ebe code end
    ...
    exit;
        ...
        if(curl_errno($ch)) {
            $log = date("Y-m-d H:i:s") . " Refund Response POS:".curl_errno($ch) . " \n\n";
            poslogs($log);
        } else {
            $log = date("Y-m-d H:i:s") . " Refund Response POS:" . $server_output . " \n\n";
            poslogs($log);
        }
...
exit;
    ...
    /* Query data log ,send to alipay*/
        ...
        if(curl_errno($ch)) {
            $log = date("Y-m-d H:i:s") . " Query Response POS:".curl_errno($ch) ."\n\n";
            poslogs($log);
        } else {
            $log = date("Y-m-d H:i:s") . " Query Response POS:" . $server_output . " \n\n";
            poslogs($log);
        }
